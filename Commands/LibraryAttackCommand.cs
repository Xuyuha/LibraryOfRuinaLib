...
//todo：骰子相关的方法
{
	...
	public async Task<LibraryAttackCommand> Execute(PlayerChoiceContext? choiceContext,CardPlay? cardPlay = null)
	{
		...
		for (int i = 0; i < attackCount; i++)
		{
			...
			IEnumerable<DamageResult> damageResults = await LibraryCreatureCmd.Damage(damageAmount: damage, choiceContext: choiceContext ?? new BlockingPlayerChoiceContext(), targets: targets, props: DamageProps, dealer: Attacker ,cardSource: ModelSource as CardModel, cardPlay: cardPlay, type : _damageType);
			AddDamageResultsInternal(damageResults);
			List<LibraryChaoResult> chaoResults = [];
			for (int j = 0 ; j < Blocks.Count ; j++){
                IEnumerable<LibraryChaoResult>? results = await LibraryCreatureCmd.ChaoDamage(damageAmount: Math.Max(0, damage - Blocks[j]), choiceContext: choiceContext ?? new BlockingPlayerChoiceContext(), target: targets[j], props: DamageProps, dealer: Attacker, cardSource: ModelSource as CardModel, cardPlay: cardPlay, type: _damageType, damageResults: damageResults);
				if(results != null)
