	...
	public async Task<LibraryAttackCommand> Execute(PlayerChoiceContext? choiceContext,CardPlay? cardPlay = null)
	{
		...
		for (int i = 0; i < attackCount; i++)
		{
			...
			for (int j = 0 ; j < Blocks.Count ; j++){
                IEnumerable<LibraryChaoResult>? results = await LibraryCreatureCmd.ChaoDamage(damageAmount: Math.Max(0, damage - Blocks[j]), choiceContext: choiceContext ?? new BlockingPlayerChoiceContext(), target: targets[j], props: DamageProps, dealer: Attacker, cardSource: ModelSource as CardModel, type: _damageType, damageResults: damageResults);
				if(results != null)
