...
internal static class LibraryChaosResistanceIconsUi
{
    ...
    public static void Refresh(NHealthBar healthBar)
    {
        ...
        var staggerPower = creature.GetPower<LibraryStaggerResistancePower>();
        State state = GetOrCreateState(healthBar);

        bool shouldShow = staggerPower != null
            && creature.IsAlive && staggerPower.MaxResistanceValue > 0;
        ...
        UpdateIcons(staggerPower!, creature, state);
    }
    ...
    private static void UpdateIcons(LibraryStaggerResistancePower power, Creature creature, State state)
    {
        for (int i = 0; i < 3; i++)
        {
            LibraryDamageKind damageKind = DisplayOrder[i];
            LibraryResistanceLevel level = power.GetChaosResistance(damageKind);
